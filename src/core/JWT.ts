...
/**
 * type - differentiate access token, refresh token
 */
const TokenErrorMessage = (type: TokenType): string => trim(`${capitalize(type)} Token is not valid`);
...
export default class JWT {
   ...
   */
  public static async validate(token: string, type: TokenType = TokenType.access): Promise<JwtPayload> {
    ...
    try {
      ...
      throw new BadTokenError(TokenErrorMessage(type));
    }
  }
   ...
   */
  public static async decode(token: string, type: TokenType = TokenType.access): Promise<JwtPayload> {
    const cert = await this.readPublicKey();
    try {
      // @ts-ignore
