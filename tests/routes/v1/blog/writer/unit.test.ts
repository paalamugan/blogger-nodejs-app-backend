...
describe('Writer blog submit routes', () => {
  ...
  it('Should send success if submit blog for id exists', async () => {
    const response = await addAuthHeaders(request.put(endpoint + BLOG_ID.toHexString()), WRITER_ACCESS_TOKEN);
    expect(response.status).toBe(200);
...
describe('Writer blog delete routes', () => {
  ...
  it('Should send success if deleting blog for id exists', async () => {
    const response = await addAuthHeaders(request.delete(endpoint + BLOG_ID.toHexString()), WRITER_ACCESS_TOKEN);
    expect(response.status).toBe(200);
...
describe('Writer blog get by id routes', () => {
  ...
  it('Should send error if author is different', async () => {
    const response = await addAuthHeaders(request.get(endpoint + BLOG_ID_2.toHexString()), WRITER_ACCESS_TOKEN);
  ...
  it('Should send success if fetching blog for id exists', async () => {
    const response = await addAuthHeaders(request.get(endpoint + BLOG_ID.toHexString()), WRITER_ACCESS_TOKEN);
    expect(response.status).toBe(200);
